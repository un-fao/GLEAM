---
title: "Overview of 'gleam' R Package"
...
description of the wider ecosystem is given in [The GLEAM Ecosystem](gleam-ecosystem.html).
...
As an alternative to the full GLEAM pipeline, you can run only the direct
livestock emissions pipeline using `run_emissions_direct()`.
This function calculates CH4 emissions from enteric fermentation and CH4 and N2O
emissions from manure management.

```{r direct-emissions-quick-start}
...
The [`run_gleam()`](../reference/run_gleam.html) function orchestrates the full
modelling pipeline, while [`run_emissions_direct()`](../reference/run_emissions_direct.html)
runs the modules needed for enteric and manure emissions. Each module can also
be run independently for custom workflows. The full pipeline consists of the
following steps:
...
`run_emissions_direct()` follows the same sequence but skips feed production
emissions (step 8). Its emissions include enteric CH₄ and manure CH₄ and N₂O,
including indirect manure N₂O. Two options control which other steps run:

- **Ration quality:** provide both `feed_rations` and `feed_params` to run step 3,
  or omit both when `cohort_level_data` already contains the six ration-quality
  columns.
- **Emission factors only:** set `emission_factors_only = TRUE` to stop after
  manure emissions (step 7), skipping production, allocation, and aggregation.
  Daily per-head emission factors are returned in `results$cohort_level_results`;
  `results$allocation_long` and `results$aggregation_results` are `NULL`.

By default, `run_emissions_direct()` also runs production, allocation, and
aggregation for its direct-emission sources. Both pipelines require all six
cohorts per herd, including when `has_herd_structure = TRUE` skips herd simulation.
...
```{r, echo=FALSE, eval = TRUE, out.width="30%", fig.cap="bmleh logo"}
