...
with ISO 14044 and the IDF Global Carbon Footprint Standard.
...
description of the wider ecosystem is given in Section 9.
...
The assessment period defines the time window over which emissions and
...
|---|--------|----------|-------------|
| 1 | Herd simulation (optional) | `run_demographic_herd_module()` | Cohort sizes, offtake numbers, herd growth rate |
| 2 | Cohort weights | `run_weights_module()` | Initial, average, and final live weights; daily weight gain |
| 3 | Ration nutritional content | `run_ration_quality_module()` | Diet gross energy, digestibility, nitrogen, ash |
| 4 | Energy requirements and ration intake | `run_metabolic_energy_req_module()` | Energy partitions (maintenance, growth, lactation, etc.); dry matter intake |
| 5 | Enteric emissions | `run_emissions_enteric_module()` | Enteric CH₄ emissions |
| 6 | Nitrogen balance | `run_nitrogen_balance_module()` | Nitrogen intake, retention, and excretion |
| 7 | Manure emissions | `run_emissions_manure_module()` | CH₄ and N₂O from manure management |
| 8 | Feed production emissions | `run_emissions_ration_module()` | GHG from feed production |
| 9 | Production outputs | `run_production_module()` | Production of milk, meat, and fibre |
| 10 | Emission allocation | `run_allocation_module()` | Biophysical allocation shares and allocated emissions per commodity |
| 11 | Aggregation and reporting | `run_aggregation_module()` | Herd-level aggregation of emissions and conversion to CO₂eq |
...
pipeline. A three-tier structure is implemented consistently throughout:
```{r }
...
```
Every public function calls its validator before any computation. Constants are centralized in `gleam_constants.R`.
...
with `data.table::fread()` as shown in the Quick Start example.
...
| `cohort_short` | Character | Sex–age cohort code (see Section 3.2) |
...
- **2026–present**: GLEAM-X initiative delivering an open-source R package and
...
- [IPCC 2019 ](https://www.ipcc-nggip.iges.or.jp/public/2019rf/index.html).  *2019 Refinement to the 2006 IPCC Guidelines for National
  Greenhouse Gas Inventories*, Volume 4, Chapter 10: Emissions from Livestock
  and Manure Management ] ()
- [IPCC 2006](https://www.ipcc-nggip.iges.or.jp/public/2006gl/index.html) . *2006 IPCC Guidelines for National Greenhouse Gas Inventories*,
  Volume 4, Chapter 10.
- [IDF 2022](https://shop.fil-idf.org/products/the-idf-global-carbon-footprint-standard-for-the-dairy-sector)  *The IDF Global Carbon Footprint Standard for the Dairy Sector*.
  Bulletin of the IDF No. 520/2022.
- [FAO LEAP](https://www.fao.org/partnerships/leap/en) guidelines for livestock supply chains (large ruminants, small
  ruminants, poultry, pigs, feed).

## GLEAM publications

- [Gerber, P.J. et al. 2013](https://www.fao.org/4/i3437e/i3437e04.pdf) . *Tackling climate change through livestock: A
  global assessment of emissions and mitigation opportunities*. FAO, Rome.
- [FAO. 2023](https://openknowledge.fao.org/handle/20.500.14283/cc9029en) *Pathways towards lower emissions — A global assessment of the
  greenhouse gas emissions and mitigation options from livestock agrifood
  systems*. FAO, Rome. DOI: 10.4060/cc9029en.
- [Wisser, D. et al.](https://doi.org/10.3390/w16121681) Water Use in Livestock Agri-Food Systems and Its Contribution to Local Water Scarcity: A Spatially Distributed Global Analysis. Water, 16(12), 1681. https://doi.org/10.3390/w16121681


# Resource partner

The development of the GLEAM ecosystem was supported by the German Federal Ministry of
Food, Agriculture, and Regional Identity  
