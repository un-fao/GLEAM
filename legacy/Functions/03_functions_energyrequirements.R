...
Dfunction_nework <- function(
    Animal_short, # CTL, BFL, CML
    cohort, # CTL, BFL
    nemain, # CTL, BFL
    hours, # CTL, BFL, CML
    draught_fraction
...
# output: MJ/head/day
Dfunction_nefibre <- function(
    Animal_short, # SHP, GTS
    cohort, # SHP, GTS
    fibre_prod # SHP, GTS
) {
...
# output: MJ/head/day
Dfunction_nepreg <- function(
    Animal_short, # CTL, BFL, CML, SHP, GTS, PGS, CHK
...
# output: fraction
Dfunction_rem <- function(
    Animal_short, # CTL, BFL, SHP, GTS
...
Dfunction_nemeat <- function(
  ...
  if (Animal_short %in% c("CTL", "BFL")) {
    ...
    #     b <- 0.01045 }
    #   if (cohort %in% c("AF", "AM", "MF2", "MF3", "MF4")) {
    #     ret <- ( ( (slaughter_weight - initial_weight) * 1000 * a ) + (initial_weight - ckg) * 1000 * b ) / slaughter_weight
    #   } else if (cohort %in% c("RF", "RM", "MF1", "MM")) {
    #     ret <- ( (slaughter_weight - ckg) * 1000 * b ) / slaughter_weight
    #   }
