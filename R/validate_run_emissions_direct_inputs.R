#' Validate inputs for run_emissions_direct
#'
#' Validates the emission-factor-only option and requires either both feed
#' input tables or all six primary ration quality columns in cohort_level_data.
#' Also validates herd structure controls, master and manure input
#' tables, cohort identifiers, calculated columns, and herd ID consistency.
#' Requires exactly one row for each of the six cohorts per herd in both
#' herd-structure modes.
#' Feed production emission factors are not required by this pipeline.
#'
#' @inheritParams run_emissions_direct
#'
#' @noRd
validate_run_emissions_direct_inputs <- function(
  ...
  # --- Emission factors only: a single non-missing logical --------------------
  if (!is.logical(emission_factors_only) || length(emission_factors_only) != 1L ||
      is.na(emission_factors_only)) {
    cli::cli_abort(
      "{.arg emission_factors_only} must be a single logical value (TRUE or FALSE)."
    )
  }

  # --- Feed tables must be supplied together or both omitted -----------------
  if (xor(is.null(feed_rations), is.null(feed_params))) {
    cli::cli_abort(
      "Provide {.arg feed_rations} and {.arg feed_params} together, or omit both
      and supply primary nutritional quality in {.arg cohort_level_data}."
    )
  }
  use_primary_ration_quality <- is.null(feed_rations) && is.null(feed_params)
  if (!use_primary_ration_quality) {
    if (!is.data.frame(feed_rations)) {
      cli::cli_abort("{.arg feed_rations} must be a data frame (e.g. data.table).")
    }
    if (!is.data.frame(feed_params)) {
      cli::cli_abort("{.arg feed_params} must be a data frame (e.g. data.table).")
    }
  }

  ration_quality_cols <- c(
  ...
  # --- Every herd must contain exactly one row for each of the six cohorts ---
  if (nrow(cohort_level_data) == 0L) {
  ...
  check_cohort_completeness(
    data.table::as.data.table(cohort_level_data), "cohort_level_data"
  )

  # Feed-derived quality must cover every requested cohort before the merge.
  if (!use_primary_ration_quality) {
    check_required_columns(feed_rations, required_cohort_cols, "feed_rations")
    requested_cohorts <- unique(
      data.table::as.data.table(cohort_level_data)[, required_cohort_cols, with = FALSE]
    )
    feed_cohorts <- unique(
      data.table::as.data.table(feed_rations)[, required_cohort_cols, with = FALSE]
    )
    missing_feed_cohorts <- data.table::fsetdiff(requested_cohorts, feed_cohorts)
    if (nrow(missing_feed_cohorts) > 0L) {
      missing_info <- missing_feed_cohorts[, paste(herd_id, species_short, cohort_short, sep = " / ")]
      cli::cli_abort(
        "Missing herd_id + species_short + cohort_short combinations in {.arg feed_rations}:
        {.val {missing_info}}. Feed rations must cover every supplied cohort."
      )
    }
  }
  ...
  # Accept herd structure inputs when the user supplies the herd structure.
  ...
  if (use_primary_ration_quality) {
    cohort_blocklist <- setdiff(cohort_blocklist, ration_quality_cols)
  }
  ...
  # --- Primary nutritional quality is required when feed tables are omitted --
  if (use_primary_ration_quality) {
    missing_quality_cols <- setdiff(ration_quality_cols, names(cohort_level_data))
    if (length(missing_quality_cols) > 0L) {
      cli::cli_abort(
        "When {.arg feed_rations} and {.arg feed_params} are omitted, supply primary
        nutritional quality in {.arg cohort_level_data}.
        Missing required columns: {.val {missing_quality_cols}}."
      )
    }
    for (col in ration_quality_cols) {
      validate_param_range(cohort_level_data[[col]], arg_name = col)
    }
  }
