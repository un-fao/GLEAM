...
test_that("validate_feed_indirect_emissions_inputs reports duplicated feed_id values", {
  rations_share <- data.table::data.table(
    herd_id = 1,
    animal = "Cattle",
    feed_name = "MAIZE",
  ...
  expect_error(
    validate_feed_indirect_emissions_inputs(rations_share, feed_indirect_emissions),
    regexp = "(Duplicated feed_id\\(s\\)|feed_emissions\\$feed_id).*must be unique"
  )
})



test_that("rejects duplicated feed_id within herd/cohort/animal", {
  rations_share <- data.table::data.table(
    herd_id = c(1, 1),
    animal = c("Cattle", "Cattle"),
    feed_name = c("MAIZE", "BARLEY"),
    feed_id = c(1, 1),  # duplicate id
    cohort_short = c("FJ", "FJ"),
    feed_ration_fraction = c(0.5, 0.5)
  )
  
  feed_emissions <- data.table::data.table(
    feed_id = c(1, 2),
    feed_name = c("MAIZE", "BARLEY"),
    co2_feed_fertilizer = c(10, 11),
    co2_feed_pesticides = c(1, 2),
  ...
  expect_error(
    ...
    regexp = "duplicated.*feed_id"
  )
})


test_that("rejects duplicated feed_name within herd/cohort/animal", {
  rations_share <- data.table::data.table(
    herd_id = c(1, 1),
    animal = c("Cattle", "Cattle"),
    feed_name = c("MAIZE", "MAIZE"),  # duplicate name
    feed_id = c(1, 2),
    cohort_short = c("FJ", "FJ"),
    feed_ration_fraction = c(0.5, 0.5)
  )
