...
S1_03_PopStructure <- function(x_start, max_years, min_lambda_change, fecF, fecM, pdea, poff, g) {
  ...
  xend <- c(Fem_B__x_fec[days_steady], Fem_J__x_fec[days_steady], Fem_S__x_fec[days_steady], Fem_A__x_fec[days_steady],
            Mal_B__x_fec[days_steady], Mal_J__x_fec[days_steady], Mal_S__x_fec[days_steady], Mal_A__x_fec[days_steady])
  ...
  share <- c(structure[1] + structure[2], structure[3:4], structure[5] + structure[6], structure[7:8])
...
S1_04_PopSize <- function(size_total, fecF, fecM, pdea, poff, g, growth_rate_pop, structure, share) {
  ...
  size_end_exact    <- c(Fem_B__x_fec[366], Fem_J__x_fec[366], Fem_S__x_fec[366], Fem_A__x_fec[366], Fem_C__x_fec[366],
                         Mal_B__x_fec[366], Mal_J__x_fec[366], Mal_S__x_fec[366], Mal_A__x_fec[366], Mal_C__x_fec[366]) 
...
S1_05_ProdOfftake <- function(size, size_end, size_avg, offtake) {
  ...
  offtake_number       <- c(sum(offtake[1:2]), offtake[3], sum(offtake[4:5]), sum(offtake[6:7]), offtake[8], sum(offtake[9:10]))
  ...
  names(output) <- c("stock_variation", "offtake_number", "offtake_share", "offtake_share_avg", "offtake_sv_number", "offtake_sv_share", "offtake_sv_share_avg")
...
S1_06_ProdCommodities <- function(dry_matter_req, liveweight, carcass, financial_value, milking_length, milk, skin, wool, manure, part_rate, prolif_rate, fem_birth_ratio, duration, S1_02_results, S1_04_results, S1_05_results) {
  ...
  financial_offtake <- sum(offtake * c(financial_value[1], financial_value[c(1:3)], financial_value[3], financial_value[4], financial_value[c(4:6)], financial_value[6]))
  ...
  plotdata <- data.frame(equivalent = c(rep("live weight (kg)", 3), rep("meat (kg)", 3), rep("financial value (currency)", 3)),
                         ...
                         value      = c(liveweight_stock, liveweight_offtake, liveweight_sv, 
  ...
  output        <- list(liveweight_stock, liveweight_offtake, liveweight_sv, 
                        meat_stock, meat_offtake, meat_sv, 
                        financial_stock, financial_offtake, financial_sv, 
                        milk_total, milk_female,
                        skin_total, wool_total, manure_total,
                        subad_adF, ad_adF, feed_req, plot_equivalents)
  names(output) <- c("liveweight_stock", "liveweight_offtake", "liveweight_sv", 
                     "meat_stock", "meat_offtake", "meat_sv", 
                     "financial_stock", "financial_offtake", "financial_sv",
                     "milk_total", "milk_female",
                     "skin_total", "wool_total", "manure_total",
