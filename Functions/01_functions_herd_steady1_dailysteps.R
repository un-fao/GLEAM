...
S1_03_PopStructure <- function(x_start, max_years, min_lambda_change, fecF, fecM, pdea, poff, g) {
  ...
  xend <- c(
    Fem_B__x_fec[days_steady], Fem_J__x_fec[days_steady], Fem_S__x_fec[days_steady], Fem_A__x_fec[days_steady],
...
S1_04_PopSize <- function(size_total, fecF, fecM, pdea, poff, g, growth_rate_pop, structure, share) {
  ...
  size_end_exact <- c(
    Fem_B__x_fec[366], Fem_J__x_fec[366], Fem_S__x_fec[366], Fem_A__x_fec[366], Fem_C__x_fec[366],
...
S1_06_ProdCommodities <- function(dry_matter_req, liveweight, carcass, financial_value, milking_length, milk, skin, wool, manure, part_rate, prolif_rate, fem_birth_ratio, duration, S1_02_results, S1_04_results, S1_05_results) {
  ...
  plotdata <- data.frame(
    equivalent = c(rep("live weight (kg)", 3), rep("meat (kg)", 3), rep("financial value (currency)", 3)),
