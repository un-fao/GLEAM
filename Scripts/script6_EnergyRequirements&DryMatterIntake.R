...
GLEAM_input_energyrequirement[, negrow := Dfunction_negrow(Animal_short, cohort, 
                                               averageLW, finalLW, initialLW, dwg, offtake_rate, 
...
GLEAM_input_energyrequirement[, nemeat := Dfunction_nemeat(Animal_short, cohort = cohort,
                                               ckg, afc, slaughterLW, initialLW), by = seq_len(nrow(GLEAM_input_energyrequirement))]
...
# View(GLEAM_input_energyrequirement[,.(Animal_short, COUNTRY, LPS, HerdType, cohort, MLK_YIELD, FR, WKG, initialLW,nelact, averageLW, nemain,   nepreg, negrow, diet_dig, rem, reg, getot, dmi)])
