...
herd.dt[, c("fecF", "fecM") :=
             S1_01_Fecundity(parturition_rate, litsize, female_birth_fraction), by = seq_len(nrow(herd.dt))]
...
vecNames.f2 <- names(unlist(S1_02_Probabilities(as.numeric(herd.dt[1,.(duration.FJ, duration.FS, duration.FA, duration.MJ, duration.MS, duration.MA)]), 
...
herd.dt[, (vecNames.f2) :=
             as.list(as.vector(
               unlist(
                 S1_02_Probabilities(duration = as.numeric(.(duration.FJ, duration.FS, duration.FA, duration.MJ, duration.MS, duration.MA)), 
...
# # #  Defining the vector names to be used for outputs, through a run of the function for the first record
vecNames.f3 <- names(unlist(S1_03_PopStructure(x_start, max_years, min_lambda_change, herd.dt[1,fecF], herd.dt[1,fecM], 
...
herd.dt[, (vecNames.f3) :=
             as.list(as.vector(
               unlist(
                 S1_03_PopStructure(x_start, max_years, min_lambda_change, fecF, fecM,
...
# # #  Defining the vector names to be used for outputs, through a run of the function for the first record
vecNames.f4 <- names(unlist(S1_04_PopSize(herd.dt[1,size_total], herd.dt[1,fecF], herd.dt[1,fecM], 
                                          as.numeric(herd.dt[1,c("pdea.FB", "pdea.FJ", "pdea.FS", "pdea.FA", "pdea.FC",  "pdea.MB",  "pdea.MJ", "pdea.MS", "pdea.MA", "pdea.MC")]), 
                                          as.numeric(herd.dt[1,c("poff.FB", "poff.FJ", "poff.FS", "poff.FA", "poff.FC",  "poff.MB",  "poff.MJ", "poff.MS", "poff.MA", "poff.MC")]), 
                                          as.numeric(herd.dt[1,c("g.FB", "g.FJ", "g.FS", "g.FA", "g.FC",  "g.MB",  "g.MJ", "g.MS", "g.MA", "g.MC")]),
...
herd.dt[, (vecNames.f4) :=
             as.list(as.vector(
               unlist(
                 S1_04_PopSize(size_total, fecF, fecM, 
                               pdea = as.numeric(.(pdea.FB, pdea.FJ, pdea.FS, pdea.FA, pdea.FC,  pdea.MB,  pdea.MJ, pdea.MS, pdea.MA, pdea.MC)), 
                               poff = as.numeric(.(poff.FB, poff.FJ, poff.FS, poff.FA, poff.FC,  poff.MB,  poff.MJ, poff.MS, poff.MA, poff.MC)), 
                               g = as.numeric(.(g.FB, g.FJ, g.FS, g.FA, g.FC,  g.MB,  g.MJ, g.MS, g.MA, g.MC)),
                               growth_rate_pop,
...
# # #  Defining the vector names to be used for outputs, through a run of the function for the first record
vecNames.f5 <- names(unlist(S1_05_ProdOfftake(
...
extra_cols <- c("share.FJ", "share.FS", "share.FA", "share.MJ", "share.MS", "share.MA",
 "growth_rate_pop",
