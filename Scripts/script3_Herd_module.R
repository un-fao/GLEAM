...
herd_dt[, (transition_cols) := as.list(
  unlist(
    compute_transition_probabilities(
      duration = as.numeric(
        .(duration.FJ, duration.FS, duration.FA, duration.MJ, duration.MS, duration.MA)
...
structure_cols <- names(
  unlist(
    simulate_steady_state_structure(
      ...
      female_fecundity = herd_dt[1, female_fecundity],
      male_fecundity = herd_dt[1, male_fecundity],
      pdea = as.numeric(
        herd_dt[1, c(
...
herd_dt[, (structure_cols) := as.list(
  unlist(
    simulate_steady_state_structure(
      x_start = x_start,
      max_years = max_years,
...
popsize_cols <- names(
  unlist(
    project_population_size(
      size_total = herd_dt[1, size_total],
      female_fecundity = herd_dt[1, female_fecundity],
      male_fecundity = herd_dt[1, male_fecundity],
      pdea = as.numeric(
        herd_dt[1, c("pdea.FB", "pdea.FJ", "pdea.FS", "pdea.FA", "pdea.FC", "pdea.MB", "pdea.MJ", 
                     "pdea.MS", "pdea.MA", "pdea.MC")]
      ),
      poff = as.numeric(
        herd_dt[1, c("poff.FB", "poff.FJ", "poff.FS", "poff.FA", "poff.FC", "poff.MB", "poff.MJ", 
                     "poff.MS", "poff.MA", "poff.MC")]
      ),
      g = as.numeric(
        herd_dt[1, c("g.FB", "g.FJ", "g.FS", "g.FA", "g.FC", "g.MB", "g.MJ", "g.MS", "g.MA", "g.MC")]
      ),
      growth_rate_pop = herd_dt[1, growth_rate_pop],
      structure = as.numeric(
        herd_dt[1, c("structure.FB", "structure.FJ", "structure.FS", "structure.FA", 
                     "structure.MB", "structure.MJ", "structure.MS", "structure.MA")]
      ),
      share = as.numeric(
        herd_dt[1, c("share.FJ", "share.FS", "share.FA", "share.MJ", "share.MS", "share.MA")]
      )
    )
  )
)

herd_dt[, (popsize_cols) := as.list(
  unlist(
    project_population_size(
...
offtake_cols <- names(
  unlist(
    summarise_offtake(
      size = as.numeric(
        herd_dt[1, c("size.FJ", "size.FS", "size.FA", "size.MJ", "size.MS", "size.MA")]
      ),
      size_end = as.numeric(
        herd_dt[1, c("size_end.FJ", "size_end.FS", "size_end.FA", "size_end.MJ", "size_end.MS", 
                     "size_end.MA")]
      ),
      size_avg = as.numeric(
        herd_dt[1, c("size_avg.FJ", "size_avg.FS", "size_avg.FA", "size_avg.MJ", "size_avg.MS", 
                     "size_avg.MA")]
      ),
      offtake = as.numeric(
        herd_dt[1, c("offtake.FB", "offtake.FJ", "offtake.FS", "offtake.FA", "offtake.FC", 
...
#fwrite(herd_dt[, ..final_cols], "Inputs/GLEAM_input_herdproc.csv")
